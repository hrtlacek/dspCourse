...
\video{
	...
	Have a look at figure \ref{fig:v_origianl}. We will take this still from a video as a reference. We send the video through a couple of filters (one at a time, not in series) and watch the output. (Sorry, it is not always exactly the same frame of the video.)\\
...
\begin{figure}[H]
	\begin{center}
		...
		\caption[Video Filters: Blur]{This effect is known as a \textit{blur}(Please ignore that it's a different frame than the still from the original). It is generalted by using a FIR lowpass in the space domain. That means, in the simplest case, each output pixel is the average of the input pixel at the same location and its neighbors. In practice, weighting functions are used to control, how much the input pixel and how much its neighbors contribute. These weighting functions are called kernels, (since this FIR filtering is the same as convolution). Typical kernels are: Gaussian, Box, the sinc function, etc. }
		\label{fig:v_blur}
	\end{center}
\end{figure}
...
In pd, we can use \pd{lores\textasciitilde }, \pd{onepole\textasciitilde}. More basic building blocks that can be used to realize a lowpass are \pd{rpole\textasciitilde }, \pd{rzero\textasciitilde }, \pd{cpole\textasciitilde }. \pd{czero\textasciitilde }.\\
As a highpass, we can just use a structure that subtracts a lowpassed version from our original signal. For example:
